...
def add_record():
   ...
   if not name or not email or not contact or not gender or not stream:
       ...
       try:
           connector.execute(
           'INSERT INTO SCHOOL_MANAGEMENT (NAME, EMAIL, PHONE_NO, GENDER, STREAM) VALUES (?,?,?,?,?)', (name, email, contact, gender, stream)
           )
           connector.commit()
...
def view_record():
   global name_strvar, email_strvar, contact_strvar, gender_strvar, stream_strvar
   ...
   #dob.set_date(date);
   stream_strvar.set(selection[5])


# Initializing the GUI window
main = Tk()
main.title('NIIT School Management System')
...
#icon = PhotoImage(file='NIIT LOGO.png')
#main.iconphoto(True, icon)
main.iconbitmap('NIIT LOGO.png')
...
Label(main, text="SCHOOL MGT APPS", font=headlabelfont, bg='SpringGreen').pack(side=TOP, fill=X)
...
lf_bg = 'MediumSpringGreen' # bg color for the left_frame
cf_bg = 'PaleGreen' # bg color for the center_frame
icon = PhotoImage(file='NIIT LOGO.ico')
